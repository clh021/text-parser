...
build: generate
	@cd cmd;CGO_ENABLED=0 GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "../bin/text-parser"
	@echo "[OK] App binary was created!"

buildcross: generate
	@cd cmd;CGO_ENABLED=0 GOARCH=amd64 GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "../bin/text-parser.amd64"
	@cp ./bin/text-parser.amd64 ./bin/text-parser.x86_64
	@echo "[OK] App amd64 binary was created!"
	@cd cmd;CGO_ENABLED=0 GOARCH=arm64 GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "../bin/text-parser.arm64"
	@cp ./bin/text-parser.arm64 ./bin/text-parser.aarch64
	@echo "[OK] App arm64 binary was created!"
	@cd cmd;CGO_ENABLED=0 GOARCH=mips64le GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "../bin/text-parser.mips64le"
